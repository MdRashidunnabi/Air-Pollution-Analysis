...
def train_best_model(X_train, X_test, y_train, y_test):
    """Train Random Forest as the best model."""
    ...
    # Save model
    joblib.dump(best_model, 'trained_models/random_forest_best_model.pkl')
...
def figure_2_negative_no2_analysis(df, output_dir='visualization_plots'):
    ...
    if len(negative_NO2) == 0:
        ...
        plt.savefig(f'{output_dir}/Figure_2_Negative_NO2_Analysis_RandomForest.png', dpi=300, bbox_inches='tight')
        plt.show()
        return
...
def figure_3_feature_importance(model, feature_names, output_dir='visualization_plots'):
    ...
    bars = plt.bar(range(len(feature_names)), importances[sort_indices], 
                   color='r', alpha=0.8, edgecolor='black', linewidth=0.5, align="center")
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}/Figure_3_Feature_Importance_RandomForest.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    # Print ranking
    print("\nFeature Ranking (Random Forest):")
    for i in range(len(feature_names)):
        print(f"{i+1:2d}. feature {feature_names[sort_indices[i]]:12s} ({importances[sort_indices[i]]:.6f})")
...
def figure_4_environmental_histograms(df, output_dir='visualization_plots'):
    ...
    colors = ["b", "r", "g", "c", "m", "k", "lime", "orange", "purple"]
    
    fig = plt.figure(figsize=(13, 25))
    
    for i, j, k in zip(cols, range(len(cols)), colors):
        plt.subplot(4, 3, j+1)
        if i in df.columns:
            ...
            try:
                ...
                plt.hist(df[i], bins=30, color=k, alpha=0.7, edgecolor='black')
                plt.axvline(df[i].mean(), linestyle="dashed", label="mean", color="k")
                plt.legend(loc="best")
                plt.title(i, color="navy")
                plt.xlabel("")
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}/Figure_4_Environmental_Histograms_RandomForest.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def figure_5_correlation_heatmap(df, output_dir='visualization_plots'):
    ...
    plt.title('Correlation Heatmap of South_Link_Road_Full Data', 
              fontsize=14, fontweight='bold', pad=20)
    plt.tight_layout()
    plt.savefig(f'{output_dir}/Figure_5_Correlation_Heatmap_RandomForest.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def figure_6_pca_analysis(df, output_dir='visualization_plots'):
    ...
    for i, var in enumerate(meteorological_params):
        ...
        ax.text(loadings[i, 0]*1.15, loadings[i, 1]*1.15, var, 
               color='g', ha='center', va='center', fontweight='bold', fontsize=11)
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}/Figure_6_PCA_Analysis_RandomForest.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def figure_7_scatterplots(df, output_dir='visualization_plots'):
    ...
    plt.suptitle('Scatterplot Analysis: NO2 vs Environmental Variables', 
                 fontsize=16, fontweight='bold')
    plt.tight_layout()
    plt.savefig(f'{output_dir}/Figure_7_Scatterplots_RandomForest.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def figure_8_actual_vs_predicted(y_test, y_pred_test, test_r2, train_r2, output_dir='visualization_plots'):
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}/Figure_8_Actual_vs_Predicted_RandomForest.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def figure_9_temporal_patterns(df, output_dir='visualization_plots'):
    ...
    for param in meteorological_params:
        ...
        plt.tight_layout()
        plt.savefig(f'{output_dir}/Figure_9_Temporal_Patterns_{param}_RandomForest.png', dpi=300, bbox_inches='tight')
        plt.show()
...
def figure_10_wind_direction_analysis(df, output_dir='visualization_plots'):
    ...
    print(f"Mean Direction: {mean_direction_deg:.1f}°")
    ...
    plt.title("Wind Direction Circular Decomposition")
    plt.tight_layout()
    plt.savefig(f'{output_dir}/Figure_10_Wind_Direction_Analysis_RandomForest.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def figure_11_seasonal_patterns(df, output_dir='visualization_plots'):
    ...
    for param in meteorological_params:
        ...
        plt.tight_layout()
        plt.savefig(f'{output_dir}/Figure_11_Seasonal_Pattern_{param}_RandomForest.png', dpi=300, bbox_inches='tight')
        plt.show()
...
def main():
    ...
    output_dir = 'visualization_results/random_forest'
